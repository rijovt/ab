                        ...
                        <form method="post" action="{{ route('employees.update', $employee) }}" autocomplete="off">
                            ...
                            <div class="pl-lg-4">
                                <div class="form-group{{ $errors->has('name') ? ' has-danger' : '' }}">
                                    ...
                                    <input type="text" name="name" id="input-name" class="form-control form-control-alternative{{ $errors->has('name') ? ' is-invalid' : '' }}" placeholder="{{ __('Name') }}" value="{{ old('name', $employee->name) }}" required autofocus>
                                    @include('alerts.feedback', ['field' => 'name'])
                                </div>
                                <div class="form-group{{ $errors->has('designation') ? ' has-danger' : '' }}">
                                    <label class="form-control-label" for="input-designation">{{ __('Job Title') }}</label>
                                    <input type="designation" name="designation" id="input-designation" class="form-control form-control-alternative{{ $errors->has('designation') ? ' is-invalid' : '' }}" placeholder="{{ __('Job Title') }}" value="{{ old('designation', $employee->designation) }}" required>
